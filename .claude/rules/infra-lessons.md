...
| 7.0 kernel = SR-IOV OK via DKMS 2026.05.06 (May 2026) | strongtz/i915-sriov-dkms `2026.05.06` adds kernel 7.0 support (PR #438, issue #429 resolved). Use it on PVE 9.2 hosts. Stale BUILD_EXCLUSIVE / xe workaround from Apr 2026 is obsolete. Build needs `proxmox-headers-7.0` installed first or DKMS skips silently. |
...
| Both hosts boot GRUB (verified 2026-05-22)   | winston = GRUB-EFI, reginald = legacy GRUB-PC (CSM, `bootctl status` says "Not booted with EFI"). systemd-boot is installed in reginald's ESP but bypassed at boot. `proxmox-boot-tool kernel pin <ver>-pve` works on both — writes `/etc/default/grub.d/proxmox-kernel-pin.cfg` + runs update-grub |
| Don't edit loader.conf on legacy-booted hosts | On reginald, edits to `/boot/efi/loader/loader.conf` `default` are ignored because the system boots via legacy CSM/GRUB-PC, not systemd-boot. Verify with `bootctl status` — "Not booted with EFI" means GRUB controls boot |
