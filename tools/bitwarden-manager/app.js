...
function getFilteredItems() {
    ...
    return data.items.filter(item => {
        ...
        if (folderId && folderId !== '__none__' && item.folderId !== folderId) return false;
        ...
        if (search) {
            ...
            if (!name.includes(search) && !username.includes(search) && !url.includes(search)) {
                return false;
            }
        }
...
function renderItems() {
    ...
    if (filtered.length === 0) {
        itemsBody.innerHTML = '<tr class="empty-state"><td colspan="7">No items match your filters</td></tr>';
        updateStats();
        return;
    }
    ...
    filtered.forEach((item, index) => {
        ...
        checkbox.addEventListener('click', (e) => handleCheckboxClick(e, item.id, index));
...
function updateHeaderCheckbox() {
    ...
    const selectedCount = filtered.filter(item => selectedIds.has(item.id)).length;
    headerCheckbox.checked = selectedCount === filtered.length;
    headerCheckbox.indeterminate = selectedCount > 0 && selectedCount < filtered.length;
}
...
function moveSelected() {
    ...
    const folderName = targetFolderId ? getFolderName(targetFolderId) : 'No Folder';
